export const tosTitle = '利用規約';
export const lead =
  'この規約は、お客様が、運営者が提供するGolfersfarmサービス（以下「本サービス」）をご利用頂く際の取扱いにつき定めるものです。本規約に同意した上で本サービスをご利用ください。';
...
export const contentLeadOne = '本規約上で使用する用語の定義は、次に掲げるとおりとします。';
export const contentLeadSix =
...
export const first = [
  '本サービス：運営者が運営するサービス及び関連するサービス',
  '本サイト：本サービスのコンテンツが掲載されたウェブサイト',
  '本コンテンツ：本サービス上で提供される文字、音、静止画、動画、ソフトウェアプログラム、コード等の総称（投稿情報を含む）',
  '利用者：本サービスを利用する全ての方',
  '登録利用者：本サイトの利用者登録が完了した方',
  'ＩＤ：本サービスの利用のために登録利用者が固有に持つ文字列',
  'パスワード：ＩＤに対応して登録利用者が固有に設定する暗号',
  '投稿：登録利用者が文章、文字列、画像、コメント等を掲載する行為',
  '投稿情報：登録利用者が投稿等により掲載した文章、文字列、画像、コメント等の情報の総称',
  '個人情報：住所、氏名、職業、電話番号等個人を特定することのできる情報の総称',
  '登録情報：登録利用者が本サイトにて登録した情報の総称（投稿情報は除く）',
...
export const second = [
  ...
  '利用者が、本規約への同意手続を行った時点で、利用者と運営者との間で、本規約の諸規定に従った利用契約が成立するものとします。',
...
export const third = [
  ...
  '運営者は、本規約を改定するときは、その内容について運営者所定の方法により利用者に通知します。',
  '前本規約の改定の効力は、運営者が前項により通知を行った時点から生じるものとします。',
...
export const sixth = [
  ...
  '運営者または第三者の財産を侵害する行為、または侵害する恐れのある行為',
  ...
  '運営者提供のインターフェース以外の方法で運営者サービスにアクセスを試みる行為',
...
export const seventh = [
  '利用者は､本サービスのコンテンツを運営者の定める範囲内でのみ使用することができるものとします｡',
  '本サービスで提供される全てのコンテンツに関する権利は運営者が有しており､利用者に対し､運営者が有する特許権､実用新案権､ 意匠権､商標権､著作権､その他知的財産権の実施または使用許諾をするものではありません｡',
  '利用者は､運営者の定める使用範囲を超えていかなる方法によっても複製､送信､譲渡（利用者同士の売買も含みます）､貸与､翻訳､翻案、無断で転載、二次使用、営利目的の使用、改変、逆アセンブル、逆コンパイル、リバースエンジニアリング等を行う事を禁止します。',
  '前項に関わらず､退会等により利用者が利用者資格を喪失した場合は､提供されたコンテンツの使用権も消滅するものとします｡',
  '利用者が、本サービス上に投稿その他の方法で送信したコンテンツ（静止画、動画、文字情報その他一切の情報）に関する著作権（著作権法第21条ないし同第28条に規定する権利を含む全ての著作権を含む。）については利用者に帰属するものとします。ただし、利用者は、コンテンツの送信時に、運営者に対し、日本国内外において、当該コンテンツを無償かつ非独占的に使用することを許諾します。',
...
export const nineth = [
  '利用者は、予め運営者の書面による承諾がない限り、本規約上の地位および本規約に基づく権利または義務の全部または一部を第三者に譲渡してはならないものとします。',
  '運営者は、本サービスの全部または一部を運営者の裁量により第三者に譲渡することができ、その場合、譲渡された権利の範囲内で利用者のアカウントを含む、本サービスに係る利用者の一切の権利が譲渡先に移転するものとします。',
];
export const tenth = [
...
export const twelfth = [
  ...
  '運営者と利用者等との間での論議・訴訟その他一切の紛争については、訴額に応じて、神戸簡易裁判所又は神戸地方裁判所を専属的合意管轄裁判所とします｡',
];
